...
export default function Homepage() {
  ...
  let { clientInformations, isLogged, setClientInformations, setIsLogged } =
  ...
  useEffect(() => {
    const request = axios.get(
      "https://repoprovas-db.herokuapp.com/tests",
      config
    );
    request.then((reply) => {
    ...
    const teacherRequest = axios.get(
      "https://repoprovas-db.herokuapp.com/teacher",
      config
    );
    ...
    const categoryRequest = axios.get(
      "https://repoprovas-db.herokuapp.com/categories",
      config
    );
  ...
  function logout() {
    const request = axios.post(
      "https://repoprovas-db.herokuapp.com/logout",
      {},
      config
    );
    request.then((reply) => {
    ...
    <IconContext.Provider value={{ className: "react-icons" }}>
      <Content>
        ...
        <LinkContainer>
          ...
          <ItemsContainer>
            <a href="#" onClick={() => setTests([...allTests])}>
              All
            </a>
            {group === "teachers" &&
              teachers.map((item) => (
                <a
                  ...
                  onClick={() =>
                    setTests([
                      ...allTests.filter(
                        (test) => test.teacherId === item.teacher
                      ),
                    ])
                  }
                >{`Teacher ${item.teacher}: ${item.disciplines} `}</a>
              ))}
            {group === "disciplines" &&
              categories.map((item) => (
                <>
                    ...
                    <a
                      ...
                      onClick={() =>
                        setTests([
                          ...allTests.filter(
                            (test) =>
                              test.categoryId === item.category &&
                              test.disciplineId === discipline.name
                          ),
                        ])
                      }
                    >{`discipline ${discipline.discipline}: ${discipline.files} `}</a>
                  ))}
                </>
              ))}
          </ItemsContainer>
